...
export default function SchedulingUnassignedJobs({
  appointments,
  loading,
}: {
  ...
  return (
    <Card className="space-y-4">
      <div>
        ...
        <h3 className="text-xl font-semibold text-navy">Unassigned jobs</h3>
      </div>
      {loading ? <p className="text-sm text-slate-500">Loading…</p> : null}
      {!loading && appointments.length === 0 ? (
        <p className="text-sm text-slate-500">All scheduled jobs have technicians assigned.</p>
      ) : null}
      <div className="space-y-2">
        {appointments.slice(0, 5).map((appointment) => (
          <div key={appointment.id} className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
