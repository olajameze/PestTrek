...
const PLAN_KEYS: MarketingPlanKey[] = marketingPlanKeys();

function planKeyFromName(name: string): MarketingPlanKey {
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-12 text-center">
          ...
          <p className="mx-auto max-w-2xl text-lg text-slate-500 sm:text-xl">
            Monthly or yearly billing. All plans include a 7 day free trial. Cancel anytime.
          </p>
          <div className="mt-8 flex justify-center">
            <BillingIntervalToggle value={billingInterval} onChange={setBillingInterval} />
          </div>
          {billingInterval === 'year' ? (
            <p className="mt-4 text-sm font-medium text-emerald-700">Yearly plans bill once per year in GBP.</p>
          ) : null}
        </div>
        <div className="grid gap-8 md:grid-cols-3">
          {pricingPlans.map((plan, idx) => {
            const planKey = PLAN_KEYS[idx] ?? planKeyFromName(plan.name);
            const amount = getMarketingPlanPriceGbp(planKey, billingInterval);
            const savings = billingInterval === 'year' ? annualPlanSavingsGbp(planKey) : 0;
            ...
            return (
              <FadeIn key={plan.name} delay={plan.isPopular ? 0.1 : 0}>
                ...
                >
                  ...
                  <div className="mb-6">
                    <div>
                      <span className="text-4xl font-black sm:text-5xl">{formatGbpPrice(amount)}</span>
                      <span className="text-sm text-slate-400">
                        {billingInterval === 'year' ? '/year' : plan.cadence}
                      </span>
                    </div>
                    {billingInterval === 'year' ? (
                      <p className="mt-1 text-sm font-medium text-slate-500">
                        {formatAnnualEffectiveMonthly(planKey)} billed annually
                      </p>
                    ) : null}
                    {billingInterval === 'year' && savings > 0 ? (
                      <p className="mt-1 text-sm font-semibold text-emerald-600">
                        Save {formatGbpPrice(savings)} vs paying monthly
                      </p>
                    ) : null}
                    {billingInterval === 'month' && pricingApproxLabels[idx] ? (
                      <p className="mt-1 text-sm font-medium text-slate-400">{pricingApproxLabels[idx]}</p>
                    ) : null}
                    {billingInterval === 'year' ? (
                      <p className="mt-1 text-sm font-medium text-slate-400">{formatGbpYearly(amount)} in GBP</p>
                    ) : null}
                  </div>
                  <Link
                    href="/auth/signup"
        ...
        <p className="mx-auto mt-12 max-w-3xl text-center text-sm leading-relaxed text-slate-500">
          ...
          {billingInterval === 'year' ? ' Yearly billing is selected at checkout in Settings or Upgrade.' : ''}
          {pricingFxNote && billingInterval === 'month' ? ` ${pricingFxNote}` : ''}
        </p>
      </div>
    </section>
  );
